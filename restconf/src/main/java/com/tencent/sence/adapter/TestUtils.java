...
public final class TestUtils {
  ...
  public static String getDocumentInPrintableForm(final Document doc) {
    try {
      ...
      transformer.transform(
          new DOMSource(requireNonNull(doc)),
  ...
  /**
   * Searches module with name {@code searchedModuleName} in {@code modules}. If module name isn't
   * specified and module set has only one element then this element is returned.
   */
  public static Module resolveModule(final String searchedModuleName, final Set<Module> modules) {
    if (modules.isEmpty()) {
      throw new RuntimeException("Modules can't be null.");
    }
  ...
  public static DataSchemaNode resolveDataSchemaNode(
      final String searchedDataSchemaName, final Module module) {
    if (module == null) {
      throw new RuntimeException("Modules can't be null.");
    }
  ...
  public static QName buildQName(
      final String name, final String uri, final String date, final String prefix) {
    try {
  ...
  public static NodeIdentifier getNodeIdentifier(
      final String localName, final String namespace, final String revision) throws ParseException {
    return new NodeIdentifier(QName.create(namespace, revision, localName));
  }

  public static NodeIdentifierWithPredicates getNodeIdentifierPredicate(
  ...
  public static NodeIdentifierWithPredicates getNodeIdentifierPredicate(
      ...
      throws ParseException {
    checkArgument(
        keysAndValues.length % 2 == 0, "number of keys argument have to be divisible by 2 (map)");
    final Map<QName, Object> predicate = new HashMap<>();

    int index = 0;
    while (index < keysAndValues.length) {
      predicate.put(
          QName.create(namespace, revision, keysAndValues[index++]), keysAndValues[index++]);
    }

    return NodeIdentifierWithPredicates.of(QName.create(namespace, revision, localName), predicate);
  }

  public static NormalizedNode<?, ?> prepareNormalizedNodeWithIetfInterfacesInterfacesData()
      throws ParseException {
    ...
    mapEntryNode.withNodeIdentifier(
        getNodeIdentifierPredicate("interface", namespace, ietfInterfacesDate, predicates));
    mapEntryNode.withChild(
        new ImmutableLeafNodeBuilder<String>()
            .withNodeIdentifier(getNodeIdentifier("name", namespace, ietfInterfacesDate))
            .withValue("eth0")
            .build());
    mapEntryNode.withChild(
        new ImmutableLeafNodeBuilder<String>()
            .withNodeIdentifier(getNodeIdentifier("type", namespace, ietfInterfacesDate))
            .withValue("ethernetCsmacd")
            .build());
    mapEntryNode.withChild(
        new ImmutableLeafNodeBuilder<Boolean>()
  ...
  public static SchemaContextHandler newSchemaContextHandler(final SchemaContext schemaContext) {
    return new SchemaContextHandler(null, null);
  }
}
