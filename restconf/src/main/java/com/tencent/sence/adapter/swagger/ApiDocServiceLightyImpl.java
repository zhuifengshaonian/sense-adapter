...
public abstract class ApiDocServiceLightyImpl implements ApiDocService {
  ...
  public synchronized Response getListOfMounts(final UriInfo uriInfo) {
    ...
    try (OutputStreamWriter streamWriter = new OutputStreamWriter(baos, StandardCharsets.UTF_8);
      ...
      for (final Map.Entry<String, Long> entry :
          mountPointSwagger.getInstanceIdentifiers().entrySet()) {
  ...
  @Override
  public synchronized Response getMountDocByModule(
      final String instanceNum, final String module, final String revision, final UriInfo uriInfo) {
    final ApiDeclaration api;
    api =
        mountPointSwagger.getMountPointApi(uriInfo, Long.parseLong(instanceNum), module, revision);
