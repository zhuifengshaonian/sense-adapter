...
public class ApiDocServiceDraft02 extends ApiDocServiceLightyImpl implements ApiDocService {
  ...
  public ApiDocServiceDraft02(
      MountPointSwaggerGeneratorDraft02 mountPointSwaggerGeneratorDraft02,
  ...
  /**
   * Generates index document for Swagger UI. This document lists out all modules with link to get
   * APIs for each module. The API for each module is served by <code> getDocByModule()</code>
   * method.
   */
  @Override
  ...
  @Override
  public synchronized Response getDocByModule(
      final String module, final String revision, final UriInfo uriInfo) {
  ...
  public synchronized Response getApiExplorer(final UriInfo uriInfo) {
    return Response.seeOther(uriInfo.getBaseUriBuilder().path("../explorer/index.html").build())
        .build();
  }
}
